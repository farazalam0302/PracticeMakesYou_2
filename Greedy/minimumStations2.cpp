...
bool myCompare(pair<int, char> &left, pair<int, char> &right) {
...
public:
  int findMinimumStations(vector<pair<int, char>> &vp) {
    int n = vp.size();
    ...
    for (int i = 0; i < n; i++) {
      if (vp[i].second == 'a') {
        platformNeed++;
        maxNeed = max(platformNeed, maxNeed);
      } else if (vp[i].second == 'd') {
        platformNeed--;
      }
    }
    return maxNeed;
...
int main(int argc, const char **argv) {
  ...
  while (t--) {
    ...
    MiniStations ms;
    cout << ms.findMinimumStations(vp) << endl;
  }
