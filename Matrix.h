...
class Matrix {
  ...
  // [ ] does C style "slicing": gives an N-1 dimensional matrix from an N
  // dimensional one row() is equivalent to [ ] column() is not (yet)
  // implemented because it requires strides. = has copy semantics ( ) and [ ]
  // are range checked slice() to give sub-ranges
 private:
...
class Matrix_base {
  ...
  // semantics Matrix_base does element-wise operations
 protected:
 ...
 public:
  ...
  template <class F>
  void base_apply(F f) {
    for (Index i = 0; i < size(); ++i) f(elem[i]);
  }
  template <class F>
  void base_apply(F f, const T& c) {
    for (Index i = 0; i < size(); ++i) f(elem[i], c);
  }
...
class Matrix<T, 1> : public Matrix_base<T> {
 ...
 public:
  ...
  // argument T f(const T&) would be a typical type for f
  {
  ...
  // construct a new Matrix with element's that are functions of a's elements:
  // does not modify a unless f has been specifically programmed to modify its
  // argument T f(const T&, const Arg&) would be a typical type for f
  {
...
class Matrix<T, 2> : public Matrix_base<T> {
 ...
 protected:
  ...
  Matrix(Index n1, Index n2, T* p)
      : Matrix_base<T>(n1 * n2, p), d1(n1), d2(n2) {
 ...
 public:
  ...
  Matrix(Row<T, 2>& a)
      : Matrix_base<T>(a.dim1() * a.dim2(), a.p), d1(a.dim1()), d2(a.dim2()) {
  ...
  // Column<T,1> column(Index n); // not (yet) implemented: requies strides and
...
class Matrix<T, 3> : public Matrix_base<T> {
 ...
 protected:
  ...
  Matrix(Index n1, Index n2, Index n3, T* p)
      : Matrix_base<T>(n1 * n2 * n3, p), d1(n1), d2(n2), d3(n3) {
    // std::cerr << "construct 3D Matrix from data\n";
  }
 ...
 public:
  ...
  Matrix(const Matrix& a)
      : Matrix_base<T>(a.size(), 0), d1(a.d1), d2(a.d2), d3(a.d3) {
    // std::cerr << "copy ctor\n";
    this->base_copy(a);
  }
  ...
  // Column<T,2> column(Index n); // not (yet) implemented: requies strides and
  // operations on columns
...
template <class T, int N>
Matrix<T, N - 1> scale_and_add(const Matrix<T, N>& a, const Matrix<T, N - 1> c,
