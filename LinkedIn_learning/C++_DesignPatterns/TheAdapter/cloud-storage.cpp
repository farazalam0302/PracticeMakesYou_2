...
public:
  bool uploadContents(const string &content) override {
    cout << "Uploading " << content.length()
         << " bytes to CloudDrive: " << endl;
...
class LegacyDriveAdapter : public CloudDrive {
private:
  unique_ptr<VirtualDrive> legacyVirtualDrive;

public:
  LegacyDriveAdapter() : legacyVirtualDrive(make_unique<VirtualDrive>()) {}
  bool uploadContents(const string &content) override {
    return legacyVirtualDrive->uploadData(content, 0);
  }
  int getFreeSpace() { return legacyVirtualDrive->usedSpace(); }
};

int main() {
  // Create an array of pointers to CloudStorage objects.
  const std::unique_ptr<CloudStorage> cloudServices[]{
      std::make_unique<CloudDrive>(), std::make_unique<FastShare>(),
      std::make_unique<LegacyDriveAdapter>()};
