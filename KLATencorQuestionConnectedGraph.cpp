...
void
displayGraph (vector<list<int>> &G)
{
  int n = G.size ();
  for (int i = 0; i < n; ++i)
    {
...
void
dfsCompute (vector<list<int>> &G_adjList, int node, vector<bool> &visited,
	    int &count)
{
  if (visited[node])
    return;
...
int
computeCost (int nodes, vector<int> &graph_from, vector<int> &graph_to)
{
