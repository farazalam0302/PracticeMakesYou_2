...
bool isValidPlacement(vector<vector<int>>& chess, int row, int column) {
...
bool solveNQueenChess(vector<vector<int>>& chess, int column) {
  if (column >= N) return true;
  for (int i = 0; i < N; ++i) {
    if (isValidPlacement(chess, i, column)) {
      chess[i][column] = 1;
      if (solveNQueenChess(chess, column + 1)) return true;
      chess[i][column] = 0;
    }
  }
  return false;
